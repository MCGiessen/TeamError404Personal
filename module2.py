...
def get_current_position_and_heading():
    ...
    """
    global update_index, head_count
    ...
    # Print heading every 10 iterations
    if head_count >= 10:
        print(f"Current Heading: {heading:.2f} degrees")
        head_count = 0  # Reset the counter
    ...
    current_position = np.around([data[update_index - 1, 2], data[update_index - 1, 0]], decimals=3)  # [z, x]
...
def compute_steering(current_position, current_heading):
    ...
    dz = target_waypoint[0] - current_position[0]  # z-axis difference
    dx = target_waypoint[1] - current_position[1]  # x-axis difference
    distance_to_waypoint = np.hypot(dx, dz)

    # Check if the waypoint is reached
    if distance_to_waypoint < 0.1:  # Consider waypoint reached if within 10 cm
    ...
    output = np.clip(heading_error / 90, -1.0, 1.0)

    steering = .4 + output*.08

    print(f"Current Pos: {current_position}, Target: {target_waypoint}, "
...
def pwm_control(throttle, steer_cmd):
    ...
    if throttle != motor.value:
        # Control PWM devices
        motor.value = throttle

    if abs(steering_input - steering.value) > 0.01:  # Avoid tiny adjustments
...
if __name__ == "__main__":
    try:
        ...
        data_thread = threading.Thread(target=data_streaming_loop, daemon=True)
        data_thread.start()
        ...
        while not shutdown_flag:
            ...
            print(f"Steering input: {steering_input}")
